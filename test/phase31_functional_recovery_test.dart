...
void main() {
  group('Phase 31 customers', () {
    test('add edit disable and reactivate customers records audit logs', () async {
      ...
      expect(logs.map((log) => log.actionType), contains('customer.reactivated'));
    });
  });

  group('Phase 31 expenses', () {
    test('validates amount and category and persists successful expense', () async {
    ...
    test('expenses do not mutate inventory and appear in daily reports', () async {
  ...
  group('Phase 31 audit logs', () {
    test('audit logs are recorded and owner-only controller is read-only', () async {
      final audit = LocalAuditLogRepository();
      await audit.record(
        AuditLogDraft(
  ...
  group('Phase 31 backup and restore', () {
    test('backup and restore preserve customers expenses and audit logs', () async {
      ...
      await source.expenses.createExpense(
        ExpenseDraft(date: _day, category: 'نولون', amountQirsh: 45000),
      );
      ...
      expect(restoredLogs.map((log) => log.actionType), contains('manual.audit'));
    });
  });
}
