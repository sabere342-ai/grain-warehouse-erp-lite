...
void main() {
  test('customer overpayment keeps receivable and advance separate', () async {
    ...
    await fixture.ledger.refundAdvance(CustomerAdvanceRefundDraft(
      ...
      createdByUserId: fixture.ownerId,
      operationRequestId: 'customer-refund-1',
    ));
    ...
    await expectLater(
      fixture.ledger.refundAdvance(CustomerAdvanceRefundDraft(
        ...
        date: DateTime(2026, 7, 15),
        createdByUserId: fixture.ownerId,
        operationRequestId: 'customer-refund-over',
      )),
      throwsA(isA<StateError>()),
    );
  });

  test('customer application reversal restores advance and receivable',
      () async {
    final fixture = await _CustomerAdvanceFixture.create();
    ...
    final application =
      ...
      date: DateTime(2026, 7, 14),
      createdByUserId: fixture.ownerId,
      operationRequestId: 'apply-reversal',
    ));
    ...
    await fixture.ledger.reverseAdvanceApplication(
      ...
      reason: 'Correction',
      operationRequestId: 'reverse-application',
    );
    ...
    expect(
        entries.last.type, CustomerAccountEntryType.advanceApplicationReversal);
  ...
  test('supplier application reversal restores advance and payable', () async {
    ...
    final approvalId = await approvals.requestApproval(
        draft: NegativeBalanceApprovalDraft(
          ...
          sourceDocumentId: 'supplier-overpay',
          sourceDocumentType: 'supplierOverpayment',
          balanceBeforeQirsh: 2000,
          expectedBalanceAfterQirsh: 800,
          reason: 'Advance',
        ),
    ...
    await ledger.createPurchaseEntry(
        purchase: PurchaseIntake(
            ...
            createdAt: DateTime(2026, 7, 14),
            stockMovementId: 'movement'));
    final application = await ledger.applyAdvance(
        SupplierAdvanceApplicationDraft(
            ...
            date: DateTime(2026, 7, 14),
            createdByUserId: owner.id,
            operationRequestId: 'supplier-apply'));
    ...
    await ledger.reverseAdvanceApplication(
        user: owner,
        applicationId: application.id,
        reason: 'Correction',
        operationRequestId: 'supplier-reverse');
    ...
    expect(entries.length, entriesBefore + 1);
    final reversal = entries
        .singleWhere((entry) => entry.id == reversed.reversalLedgerEntryId);
  ...
  test('concurrent customer application reversals apply once', () async {
    ...
    await fixture.ledger.createCreditSaleEntry(
        sale: SaleRecord(
            ...
            customerId: fixture.customerId),
        customerId: fixture.customerId);
    final application = await fixture.ledger.applyAdvance(
        CustomerAdvanceApplicationDraft(
            ...
            date: DateTime(2026, 7, 14),
            createdByUserId: fixture.ownerId,
            operationRequestId: 'concurrent-apply'));
    ...
    Future<Object> attempt(String requestId) async {
      try {
        return await fixture.ledger.reverseAdvanceApplication(
            user: fixture.ownerUser,
            applicationId: application.id,
            reason: 'Correction',
            operationRequestId: requestId);
    ...
    expect(
        entries
            .where((entry) =>
                entry.type ==
                CustomerAccountEntryType.advanceApplicationReversal)
            .length,
        1);
    expect((await fixture.ledger.listAdvanceApplications()).single.isReversed,
        isTrue);
  });
  ...
  test('concurrent supplier application reversals apply once', () async {
    ...
    final approvalId = await approvals.requestApproval(
        draft: NegativeBalanceApprovalDraft(
            ...
            amountQirsh: 200,
            operationType: NegativeBalanceOperationType.supplierOverpayment,
            sourceDocumentId: 'supplier-concurrent-overpay',
            sourceDocumentType: 'supplierOverpayment',
            balanceBeforeQirsh: 2000,
            expectedBalanceAfterQirsh: 800,
            reason: 'Advance'),
    ...
    await ledger.createPurchaseEntry(
        purchase: PurchaseIntake(
            id: 'supplier-concurrent-payable',
            supplierId: supplier.id,
            productId: 'product',
            quantityKg: 1,
            entryUnit: GrainUnit.kilogram,
    ...
    final application = await ledger.applyAdvance(
        SupplierAdvanceApplicationDraft(
            ...
            date: DateTime(2026, 7, 14),
            createdByUserId: owner.id,
            operationRequestId: 'supplier-concurrent-apply'));
    Future<Object> attempt(String requestId) async {
      try {
        return await ledger.reverseAdvanceApplication(
            user: owner,
            applicationId: application.id,
            reason: 'Correction',
            operationRequestId: requestId);
    ...
    final results = await Future.wait([
      attempt('supplier-concurrent-reversal-1'),
      attempt('supplier-concurrent-reversal-2')
    ]);
    expect(results.whereType<SupplierAdvanceApplication>().length, 1);
    expect(results.whereType<StateError>().length, 1);
    expect(await ledger.remainingAdvanceQirsh(advance.id), 200);
    expect(await ledger.balanceForSupplier(supplier.id), 100);
    final entries = await ledger.listEntries();
    final reversals = entries
        .where((entry) =>
            entry.type == SupplierAccountEntryType.advanceApplicationReversal)
        .toList();
    expect(reversals, hasLength(1));
    expect(reversals.single.debitAmountQirsh, 100);
    expect(reversals.single.creditAmountQirsh, 0);
  });

  test(
      'customer and supplier application reversal request namespaces are isolated',
      () async {
    final fixture = await _NamespaceIsolationFixture.create();

    expect(fixture.customerAdvance.id, isNot(fixture.supplierAdvance.id));
    expect(
        fixture.customerApplication.id, isNot(fixture.supplierApplication.id));
    expect(fixture.customerId, isNot(fixture.supplierId));

    final customerEntriesBefore =
        (await fixture.customerLedger.listEntries()).length;
    final supplierEntriesBefore =
        (await fixture.supplierLedger.listEntries()).length;
    final customerAdvanceBefore = await fixture.customerLedger
        .remainingAdvanceQirsh(fixture.customerAdvance.id);
    final supplierAdvanceBefore = await fixture.supplierLedger
        .remainingAdvanceQirsh(fixture.supplierAdvance.id);
    final customerBalanceBefore =
        await fixture.customerLedger.balanceForCustomer(fixture.customerId);
    final supplierBalanceBefore =
    ...
    final customerResult =
        await fixture.customerLedger.reverseAdvanceApplication(
      user: fixture.ownerUser,
    ...
    final supplierResult =
        await fixture.supplierLedger.reverseAdvanceApplication(
      user: fixture.ownerUser,
    ...
    expect(
        await fixture.customerLedger
            .remainingAdvanceQirsh(fixture.customerAdvance.id),
        200);
    expect(
        await fixture.supplierLedger
            .remainingAdvanceQirsh(fixture.supplierAdvance.id),
        200);
    expect(await fixture.customerLedger.balanceForCustomer(fixture.customerId),
        100);
    expect(await fixture.supplierLedger.balanceForSupplier(fixture.supplierId),
        100);
    expect(
    ...
    final customerReversals = customerEntries
        .where(
          (e) => e.type == CustomerAccountEntryType.advanceApplicationReversal,
        )
        .toList();
    final supplierReversals = supplierEntries
        .where(
    ...
    expect(
        customerEntry.sourceDocumentType, 'customerAdvanceApplicationReversal');
    expect(customerEntry.sourceDocumentId, fixture.customerApplication.id);
    expect(
        supplierEntry.sourceDocumentType, 'supplierAdvanceApplicationReversal');
    expect(supplierEntry.sourceDocumentId, fixture.supplierApplication.id);

    expect(customerEntry.id, isNot(supplierEntry.id));

    final customerApplications =
        await fixture.customerLedger.listAdvanceApplications();
    ...
    final supplierApplications =
        await fixture.supplierLedger.listAdvanceApplications();
    ...
    final supplierOriginalEntries = supplierEntries
        .where(
    ...
    expect(supplierOriginalEntries.single.sourceDocumentType,
        'supplierAdvanceApplication');

    final customerOriginalEntries = customerEntries
        .where(
    ...
    final customerFinancialEntries =
        await fixture.accounts.statementForAccount(fixture.financialAccount.id);
    ...
    expect(replayCustomerResult.reversalLedgerEntryId,
        customerResult.reversalLedgerEntryId);
    expect(
        (await fixture.customerLedger.listAdvanceApplications())
            .single
    ...
    expect(replaySupplierResult.reversalLedgerEntryId,
        supplierResult.reversalLedgerEntryId);
    expect(
        (await fixture.supplierLedger.listAdvanceApplications())
            .single
            .reversalLedgerEntryId,
    ...
    expect(await fixture.supplierLedger.balanceForSupplier(fixture.supplierId),
        100);
    expect((await fixture.customerLedger.listEntries()).length,
        customerEntriesBefore + 1);
    expect((await fixture.supplierLedger.listEntries()).length,
        supplierEntriesBefore + 1);
    expect(
        await fixture.accounts
            .currentBalanceForAccount(fixture.financialAccount.id),
        financialBalanceBefore);
    ...
    expect(customerReversalAudits.single.referenceId,
  ...
  test('customer refund reversal restores advance and financial balance',
    ...
    final advanceBefore =
        await fixture.ledger.remainingAdvanceQirsh(fixture.advance.id);
    final financialBefore =
        await fixture.accounts.currentBalanceForAccount(fixture.account.id);
    ...
    final result = await fixture.ledger.reverseAdvanceRefund(
      ...
      operationRequestId: 'customer-refund-reverse-1',
    );
    ...
    final reversalFinancials = financialStatement.lines
        .where(
          (l) =>
              l.entry.sourceType ==
    ...
    expect(reversalFinancials.single.entry.direction,
        FinancialAccountEntryDirection.inflow);
    expect(reversalFinancials.single.entry.amountQirsh, 100);
    expect(reversalFinancials.single.entry.reversalOf,
        fixture.refund.financialEntryId);
    ...
    expect(
        replayResult.reversalFinancialEntryId, result.reversalFinancialEntryId);
    expect(await fixture.accounts.currentBalanceForAccount(fixture.account.id),
        2200);
    expect((await fixture.ledger.listEntries()).length, entriesBefore);
    expect((await fixture.audit.listLogs()).length, auditBefore + 2);
  ...
  test('supplier refund reversal restores advance and financial balance',
    ...
    final reversalApprovalId = await fixture.approvals.requestApproval(
        draft: NegativeBalanceApprovalDraft(
          ...
          approvedByOwnerUserId: fixture.ownerUser.id,
          accountId: fixture.account.id,
          amountQirsh: 100,
          operationType:
              NegativeBalanceOperationType.supplierAdvanceRefundReversal,
          sourceDocumentId: 'supplier-refund-reverse-1',
          sourceDocumentType: 'supplierAdvanceRefundReversal',
          balanceBeforeQirsh: -100,
          expectedBalanceAfterQirsh: -200,
          reason: 'Reversal',
          authorizationContext:
              NegativeBalanceApprovalContext.supplierAdvanceRefundReversal(
                  supplierId: fixture.refund.supplierId,
                  advanceId: fixture.refund.advanceId,
                  refundId: fixture.refund.id),
        ),
        ownerPhone: '01000000082',
        ownerPassword: 'secret');
    ...
    final result = await fixture.ledger.reverseAdvanceRefund(
      ...
      refundId: fixture.refund.id,
      reason: 'Test reversal',
      operationRequestId: 'supplier-refund-reverse-1',
  ...
  test('concurrent supplier refund reversals apply once', () async {
    ...
    final reversalApprovalId1 = await fixture.approvals.requestApproval(
        draft: NegativeBalanceApprovalDraft(
          requestedByUserId: fixture.ownerUser.id,
          approvedByOwnerUserId: fixture.ownerUser.id,
          accountId: fixture.account.id,
        ...
        ownerPhone: '01000000082',
        ownerPassword: 'secret');
    final reversalApprovalId2 = await fixture.approvals.requestApproval(
        draft: NegativeBalanceApprovalDraft(
          requestedByUserId: fixture.ownerUser.id,
          approvedByOwnerUserId: fixture.ownerUser.id,
          accountId: fixture.account.id,
    ...
    final results = await Future.wait([
      attempt(firstId, reversalApprovalId1),
      attempt(secondId, reversalApprovalId2)
    ]);
  ...
  test('customer and supplier refund reversal request namespaces are isolated',
      () async {
    ...
    final customerResult = await fixture.customerLedger.reverseAdvanceRefund(
      user: fixture.ownerUser,
      refundId: fixture.customerRefund.id,
    ...
    final supplierResult = await fixture.supplierLedger.reverseAdvanceRefund(
      user: fixture.ownerUser,
      refundId: fixture.supplierRefund.id,
      reason: 'Namespace isolation',
      operationRequestId: sharedRequestId,
    );
    ...
    expect(customerResult.reversalFinancialEntryId,
        isNot(supplierResult.reversalFinancialEntryId));
    ...
    final customerFinancial = (await fixture.accounts
            .statementForAccount(fixture.financialAccount.id))
        .lines
        .where(
          (l) =>
              l.entry.sourceType ==
        ...
        .toList();
    final supplierFinancial = (await fixture.accounts
            .statementForAccount(fixture.financialAccount.id))
        .lines
        .where(
          (l) =>
    ...
    expect(customerFinancial.single.entry.direction,
        FinancialAccountEntryDirection.inflow);
    expect(supplierFinancial.single.entry.direction,
        FinancialAccountEntryDirection.outflow);
    expect(customerFinancial.single.entry.reversalOf,
        fixture.customerRefund.financialEntryId);
    expect(supplierFinancial.single.entry.reversalOf,
        fixture.supplierRefund.financialEntryId);

    expect((await fixture.customerLedger.listEntries()).length,
        customerEntriesBefore);
    expect((await fixture.supplierLedger.listEntries()).length,
  ...
  test('customer refund reversal rejects non-existent refund', () async {
    ...
    await expectLater(
      fixture.ledger.reverseAdvanceRefund(
        ...
        reason: 'Bad id',
        operationRequestId: 'bad-refund-id',
      ),
      throwsA(isA<StateError>()),
    );
  });
  ...
  test('customer refund reversal rejects already reversed refund', () async {
    ...
    await fixture.ledger.reverseAdvanceRefund(
      ...
      refundId: fixture.refund.id,
      reason: 'First',
      operationRequestId: 'first-reverse',
    );
    await expectLater(
      fixture.ledger.reverseAdvanceRefund(
        user: fixture.ownerUser,
        refundId: fixture.refund.id,
        reason: 'Second',
        operationRequestId: 'second-reverse',
      ),
      throwsA(isA<StateError>()),
    );
  });
...
class _CustomerRefundReversalFixture {
  ...
  static Future<_CustomerRefundReversalFixture> create() async {
    ...
    final auth = LocalAuthRepository.empty();
    final owner = await auth.createFirstOwner(
        name: 'Owner', phone: '01000000081', password: 'secret');
    final approvals = NegativeBalanceApprovalService(
        authRepository: auth,
        approvalRepository: LocalNegativeBalanceApprovalRepository(),
        auditLogRepository: audit);
    ...
    final customer = await customers
        .createCustomer(const CustomerDraft(name: 'Refund Customer'));
    ...
    final cApprovalId = await approvals.requestApproval(
        draft: NegativeBalanceApprovalDraft(
          requestedByUserId: owner.id,
          approvedByOwnerUserId: owner.id,
          accountId: account.id,
          ...
          sourceDocumentId: 'refund-customer-overpay',
          sourceDocumentType: 'customerOverpayment',
          balanceBeforeQirsh: 1000,
          expectedBalanceAfterQirsh: 2200,
          reason: 'Advance',
        ),
        ownerPhone: '01000000081',
        ownerPassword: 'secret');
    ...
    final refund = await ledger.refundAdvance(CustomerAdvanceRefundDraft(
      ...
      date: DateTime(2026, 7, 14),
      createdByUserId: owner.id,
      operationRequestId: 'refund-customer-1',
    ));
...
class _SupplierRefundReversalFixture {
  ...
  static Future<_SupplierRefundReversalFixture> create() async {
    ...
    final auth = LocalAuthRepository.empty();
    final owner = await auth.createFirstOwner(
        name: 'Owner', phone: '01000000082', password: 'secret');
    final approvals = NegativeBalanceApprovalService(
        authRepository: auth,
        approvalRepository: LocalNegativeBalanceApprovalRepository(),
        auditLogRepository: audit);
    ...
    final supplier = await suppliers
        .createSupplier(const SupplierDraft(name: 'Refund Supplier'));
    final ledger = LocalSupplierAccountRepository(
        supplierRepository: suppliers,
        auditLogRepository: audit,
        financialAccountRepository: accounts,
    ...
    await ledger.createOpeningBalanceEntry(
        supplierId: supplier.id, amountQirsh: 1000, createdByUserId: owner.id);
    final overpayApprovalId = await approvals.requestApproval(
        draft: NegativeBalanceApprovalDraft(
          requestedByUserId: owner.id,
          approvedByOwnerUserId: owner.id,
          accountId: account.id,
          amountQirsh: 200,
          operationType: NegativeBalanceOperationType.supplierOverpayment,
          sourceDocumentId: 'refund-supplier-overpay',
          sourceDocumentType: 'supplierOverpayment',
          balanceBeforeQirsh: 1000,
          expectedBalanceAfterQirsh: -200,
          reason: 'Advance',
        ...
        ownerPhone: '01000000082',
        ownerPassword: 'secret');
    final settlementApprovalId = await approvals.requestApproval(
        draft: NegativeBalanceApprovalDraft(
          requestedByUserId: owner.id,
          approvedByOwnerUserId: owner.id,
          accountId: account.id,
    ...
    final refund = await ledger.refundAdvance(SupplierAdvanceRefundDraft(
      ...
      date: DateTime(2026, 7, 14),
      createdByUserId: owner.id,
      operationRequestId: 'refund-supplier-1',
    ));
    expect(await ledger.remainingAdvanceQirsh(advance.id), 100);
...
class _BothRefundReversalFixture {
  ...
  static Future<_BothRefundReversalFixture> create() async {
    ...
    final auth = LocalAuthRepository.empty();
    final owner = await auth.createFirstOwner(
        name: 'Owner', phone: '01000000083', password: 'secret');
    final approvals = NegativeBalanceApprovalService(
        authRepository: auth,
        approvalRepository: LocalNegativeBalanceApprovalRepository(),
        auditLogRepository: audit);
    ...
    final customers = LocalCustomerRepository(auditLogRepository: audit);
    final customer = await customers
        .createCustomer(const CustomerDraft(name: 'NS Refund Cust'));
    ...
    final cApprovalId = await approvals.requestApproval(
        draft: NegativeBalanceApprovalDraft(
          ...
          amountQirsh: 200,
          operationType: NegativeBalanceOperationType.customerOverpayment,
          sourceDocumentId: 'ns-refund-cust',
          sourceDocumentType: 'customerOverpayment',
          balanceBeforeQirsh: 2000,
          expectedBalanceAfterQirsh: 3200,
          reason: 'Advance',
        ),
        ownerPhone: '01000000083',
        ownerPassword: 'secret');
    ...
    final customerRefund =
        await customerLedger.refundAdvance(CustomerAdvanceRefundDraft(
      advanceId: customerAdvance.id,
      amountQirsh: 100,
      date: DateTime(2026, 7, 14),
      createdByUserId: owner.id,
      operationRequestId: 'ns-refund-cust-1',
    ));

    final balanceAfterCustomer =
        await accounts.currentBalanceForAccount(account.id);
    final suppliers = LocalSupplierRepository();
    final supplier = await suppliers
        .createSupplier(const SupplierDraft(name: 'NS Refund Supp'));
    ...
    await supplierLedger.createOpeningBalanceEntry(
        supplierId: supplier.id, amountQirsh: 1000, createdByUserId: owner.id);
    final sApprovalId = await approvals.requestApproval(
        draft: NegativeBalanceApprovalDraft(
          requestedByUserId: owner.id,
          approvedByOwnerUserId: owner.id,
          accountId: account.id,
          amountQirsh: 200,
          operationType: NegativeBalanceOperationType.supplierOverpayment,
          sourceDocumentId: 'ns-refund-supp',
          sourceDocumentType: 'supplierOverpayment',
          balanceBeforeQirsh: balanceAfterCustomer,
          expectedBalanceAfterQirsh: balanceAfterCustomer - 1200,
          reason: 'Advance',
        ),
        ownerPhone: '01000000083',
        ownerPassword: 'secret');
    ...
    final supplierRefund =
        await supplierLedger.refundAdvance(SupplierAdvanceRefundDraft(
      advanceId: supplierAdvance.id,
      amountQirsh: 100,
      date: DateTime(2026, 7, 14),
      createdByUserId: owner.id,
      operationRequestId: 'ns-refund-supp-1',
    ));
...
class _CustomerAdvanceFixture {
  ...
  static Future<_CustomerAdvanceFixture> create() async {
    ...
    final customers = LocalCustomerRepository(auditLogRepository: audit);
    final customer =
        await customers.createCustomer(const CustomerDraft(name: 'Customer'));
    final ledger = LocalCustomerAccountRepository(
      customerRepository: customers,
      auditLogRepository: audit,
      financialAccountRepository: accounts,
    ...
    required int expectedBalanceAfterQirsh,
      ...
      approvals.requestApproval(
        draft: NegativeBalanceApprovalDraft(
          ...
          sourceDocumentType: 'customerOverpayment',
          balanceBeforeQirsh: 1000,
          expectedBalanceAfterQirsh: expectedBalanceAfterQirsh,
...
class _NamespaceIsolationFixture {
  ...
  static Future<_NamespaceIsolationFixture> create() async {
    ...
    final customers = LocalCustomerRepository(auditLogRepository: audit);
    final customer = await customers
        .createCustomer(const CustomerDraft(name: 'NS Customer'));
    final customerLedger = LocalCustomerAccountRepository(
      customerRepository: customers,
      auditLogRepository: audit,
      financialAccountRepository: accounts,
    ...
    final customerApprovalId = await approvals.requestApproval(
      draft: NegativeBalanceApprovalDraft(
        ...
        amountQirsh: 200,
        operationType: NegativeBalanceOperationType.customerOverpayment,
        sourceDocumentId: 'ns-customer-overpay',
        sourceDocumentType: 'customerOverpayment',
        balanceBeforeQirsh: 2000,
        expectedBalanceAfterQirsh: 3200,
        reason: 'Advance',
      ),
      ownerPhone: '01000000099',
      ownerPassword: 'secret',
    );
    ...
    final suppliers = LocalSupplierRepository();
    final supplier = await suppliers
        .createSupplier(const SupplierDraft(name: 'NS Supplier'));
    final supplierLedger = LocalSupplierAccountRepository(
      supplierRepository: suppliers,
      auditLogRepository: audit,
      financialAccountRepository: accounts,
    ...
    final balanceAfterCustomerCollection =
        await accounts.currentBalanceForAccount(financialAccount.id);
    final supplierApprovalId = await approvals.requestApproval(
      draft: NegativeBalanceApprovalDraft(
        ...
        amountQirsh: 200,
        operationType: NegativeBalanceOperationType.supplierOverpayment,
        sourceDocumentId: 'ns-supplier-overpay',
        sourceDocumentType: 'supplierOverpayment',
