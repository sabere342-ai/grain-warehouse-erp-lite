...
void main() {
  ...
  group('Post-Expense-Analysis advances/refunds PDF logo query migration', () {
    ...
    testWidgets('valid metadata loads locator identity before exact query once',
      ...
      final queryRepository = _QueryBusinessIdentityRepositorySpy(
        failure: StateError('intentional advances/refunds query seam stop'),
        events: events,
      );
    ...
    testWidgets('query failure retains the existing PDF failure snackbar',
      ...
      final queryRepository = _QueryBusinessIdentityRepositorySpy(
        failure: StateError('advances/refunds query failure'),
      );
      ...
      expect(
          find.textContaining('advances/refunds query failure'), findsNothing);
      expect(locatorRepository.identityReads, 1);
      expect(locatorRepository.directLogoReads, 0);
      expect(queryRepository.logoReads, 1);
      expect(queryRepository.managedFileNames, [_managedFileName]);
  ...
  group('Post-Expense-Analysis advances/refunds source and architecture guards',
      () {
    test('only the selected export block moves to the existing query', () {
      ...
      expect(source, contains('_service.getAdvancesAndRefundsReport'));
      expect(
        source,
        contains(
          'FinancialReportCsvExporter.exportAdvancesAndRefundsReport',
        ),
      );
      ...
      expect(source, contains('partyTypeFilter: _partyTypeFilter'));
      expect(source, contains('entityIdFilter: _entityIdFilter'));
      expect(source, contains('supplierLookup: _supplierLookup'));
    });

    test(
        'live inventories and direct-read sets have only the Advances / Refunds delta',
        () {
...
const _managedFileName = 'post-expense-analysis-advances-refunds-logo.png';

const _identityWithLogo = BusinessIdentity(
  establishmentName: 'Post-Expense-Analysis advances/refunds warehouse',
  logo: LogoMetadata(
    managedFileName: _managedFileName,
    mimeType: 'image/png',
    sha256: 'post-expense-analysis-advances-refunds-logo',
    byteLength: 3,
    width: 1,
    height: 1,
  ),
...
const _identityWithInvalidLogo = BusinessIdentity(
  establishmentName: 'Post-Expense-Analysis advances/refunds invalid logo',
  logo: LogoMetadata(
    managedFileName: '',
    mimeType: 'image/png',
    sha256: 'post-expense-analysis-advances-refunds-invalid-logo',
    byteLength: 1,
    width: 1,
    height: 1,
  ),
