...
import 'package:grain_warehouse_erp_lite/features/ai_assistant/services/customer_collections_by_financial_account_report_reader.dart';

/// Read-only pass-through of the canonical customer-collections account report.
final class CustomerCollectionsByFinancialAccountTool implements AiTool {
  const CustomerCollectionsByFinancialAccountTool({
    required CustomerCollectionsByFinancialAccountReportReader reader,
    required AppUser? caller,
  })  : _reader = reader,
        _caller = caller;

  final CustomerCollectionsByFinancialAccountReportReader _reader;
  final AppUser? _caller;

  @override
  String get id => 'financial_customer_collections_by_account';

  @override
  String get name => 'Customer Collections by Financial Account';

  @override
  String get description =>
      'Shows canonical customer collections for one financial account and inclusive period.';
  ...
  Future<AiToolResult> execute(
    ...
    if (parameters.keys.any((key) => !supportedKeys.contains(key)) ||
      ...
      throw const AiToolValidationException(
        ...
        errors: [
          AiValidationError(
            field: 'parameters',
            message: 'Use only the approved customer-collections parameters.',
          ),
        ],
      );
    }
    ...
    final customerId = parameters['customerId'];
    if (customerId != null &&
        (customerId is! String || customerId.trim().isEmpty)) {
      throw const AiToolValidationException(
        message: 'customerId must be a non-empty string when provided.',
        errors: [
          AiValidationError(
            field: 'customerId',
            message: 'Use a non-empty customer identifier.',
          ),
        ],
      );
    }
    ...
    final report =
        await _reader.loadCustomerCollectionsByFinancialAccountReport(
    ...
    return AiToolResult(
      messages: const [
        'Customer collections by financial account report loaded.'
      ],
      data: report,
    );
  }
