...
/// Read-only pass-through of the canonical current-month financial inflows.
final class FinancialInflowsSummaryTool implements AiTool {
  const FinancialInflowsSummaryTool({
    required FinancialInflowsReportReader reader,
    required AppUser? caller,
  })  : _reader = reader,
        _caller = caller;

  final FinancialInflowsReportReader _reader;
  final AppUser? _caller;

  @override
  String get id => 'financial_inflows_summary';

  @override
  String get name => 'Financial inflows summary';

  @override
  String get description =>
      'Shows the canonical current-month financial inflows summary.';
  ...
  Future<AiToolResult> execute(
    ...
    final report = await _reader.loadFinancialInflowsReport();
    return AiToolResult(
      messages: const ['Financial inflows report loaded.'],
      data: report,
    );
  }
}
