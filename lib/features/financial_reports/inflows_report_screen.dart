...
class InflowsReportScreen extends StatefulWidget {
  const InflowsReportScreen({super.key});

  @override
  State<InflowsReportScreen> createState() => _InflowsReportScreenState();
}

class _InflowsReportScreenState extends State<InflowsReportScreen> {
  ...
  Future<void> _applyFilters() async {
    ...
    try {
      _report = await _service.inflowsReport(
        fromDate: _fromDate,
        toDate: _toDate,
      ...
      body: ListView(
        ...
        children: [
          GhalalPageHeader(
            ...
            subtitle: 'جميع الحركات المالية الواردة خلال الفترة المحددة.',
          ...
          if (_loading)
            ...
            const GhalalEmptyState(
              title: 'لا توجد تدفقات داخلة',
              message: 'لا توجد حركات مالية واردة في الفترة المحددة.',
              icon: Icons.arrow_downward_rounded,
            )
          else ...[
    ...
    return PremiumCard(
      child: Column(
        ...
        children: [
          ...
          Row(
            children: [
              _summaryItem('إجمالي التدفقات الداخلة',
  ...
  Future<void> _exportCsv() async {
    if (_report == null) return;
    try {
      final file = await FinancialReportCsvExporter.exportInflowsReport(
        report: _report!,
      );
      await _showExportResult(file);
    } catch (e) {
